...
alpha = 0.1  # 学習率
gamma = 0.9  # 割引率
epsilon = 0.2  # 初期探索率（ε-グリーディ）
num_episodes = 50000  # 学習エピソード数（収束性向上のため増加）
...
def simulate_sales(pA, pB, t, seats_A_left, seats_B_left, customers_left):
    P_A, P_B, P_0 = choice_probabilities(pA, pB, t)
    # 購入枚数は残在庫と残顧客数で制限
    max_n_A = min(seats_A_left, customers_left)
    max_n_B = min(seats_B_left, customers_left - max_n_A)  # 顧客数の重複を避ける
...
for t in range(1, days + 1):
    # 代表的な状態（初期在庫と中間在庫、顧客数はシミュレーションで変化）
